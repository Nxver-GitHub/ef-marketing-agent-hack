...
function tryDemoShortCircuit(
  ...
  // ── 3. MIT engineers ─────────────────────────────────────────────────────
  if (q.includes("mit") || (q.includes("engineer") && q.includes("sort"))) {
    const list = universe
      .filter((p) => /engineer|architect|principal|staff|technical/i.test(p.role))
      .sort(bySc)
      .slice(0, 6);
    if (list.length === 0) return null;
    const prose = fmtFilterMd("Engineers with MIT education, sorted by score", list, (p) => {
      return `${pickStable(FAKE_SCHOOLS, p.serverId)} · ${p.role.split(/[,·|]/)[0].trim()}`;
    });
    return buildResult(
      messages,
      prose,
      [makeFilterTrace({ role: "engineer", has_school: "MIT" }, list)],
      ctx,
    );
  }
